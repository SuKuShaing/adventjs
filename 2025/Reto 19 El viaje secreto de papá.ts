...
export function revealSantaRoute(routes: string[][]): string[] {
    const rutaFinal: string[] = [];

    rutaFinal.push(routes[0][0], routes[0][1]);

    let EnBusqueda = true;
    let ultimoElemento = rutaFinal[rutaFinal.length - 1];

    while (EnBusqueda) {
        for (const route of routes) {
            if (route[0] == rutaFinal[rutaFinal.length - 1]) {
                rutaFinal.push(route[1]);
            }
        }

        if (ultimoElemento == rutaFinal[rutaFinal.length - 1]) {
            break;
        }

        ultimoElemento = rutaFinal[rutaFinal.length - 1];
    }

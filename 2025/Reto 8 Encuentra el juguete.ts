...
export function findUniqueToy(toy: string): string {
    const nombreEnMinuscula = toy.toLowerCase();

    // copiamos el array de palabras para modificarlo
    const copiaDeNombreEnMinuscula = [...nombreEnMinuscula];

    for (let i = 0; i < nombreEnMinuscula.length; i++) {
        const letraAEvaluar = nombreEnMinuscula[i];

        const restoDeLaPalabra = copiaDeNombreEnMinuscula.toSpliced(i, 1);

        if (!restoDeLaPalabra.includes(letraAEvaluar)) {
            console.log("🚀 ~ findUniqueToy ~ toy[i]:", toy[i]);
            return toy[i];
        }
    }

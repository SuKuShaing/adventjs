...
export function packGifts(gifts: Gifts, maxWeight: MaxWeight): Result {
    if (gifts.some((regalo) => regalo > maxWeight)) return null;

    let trineos = 0;
    let cargaTrineo = [];

    for (let i = 0; i < gifts.length; i++) {
        const cuantoTiene = cargaTrineo.reduce(
            (acumulador, valorActual) => acumulador + valorActual,
            0
        );
        console.log(cuantoTiene);

        if (cuantoTiene + gifts[i] > maxWeight) {
            trineos++;
            cargaTrineo = [];
            cargaTrineo.push(gifts[i]);
        } else {
            cargaTrineo.push(gifts[i]);
            console.log(cargaTrineo);
        }
    }

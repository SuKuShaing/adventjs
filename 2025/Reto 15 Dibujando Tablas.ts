...
export function drawTable(data: Data, sortBy: SortBy): string {
        ...
        anchoMaximo.push(
            tabla[i].reduce(
                (acumulador, ValorActual) =>
                    (ValorActual.toString().length as number) >
                    (acumulador as number)
    ...
    for (let filas = 0; filas < tabla[0].length; filas++) {
        ...
        for (let columnas = 0; columnas < tabla.length; columnas++) {
                ...
                if (filas === 0 || filas === 1) {
                    for (
                        ...
                        diagramaTabla +=
                            `+` +
                            "-".repeat(
                                (anchoMaximo[filasSeparadora] as number) + 2
                            );
                    }
                    diagramaTabla += `+\n`;
                    yaEntre = true;
            ...
            diagramaTabla +=
                ...
                " ".repeat(
                    (anchoMaximo[columnas] as number) -
    ...
    for (
        ...
        diagramaTabla +=
            `+` + "-".repeat((anchoMaximo[filasSeparadora] as number) + 2);
    }
    diagramaTabla += `+`;

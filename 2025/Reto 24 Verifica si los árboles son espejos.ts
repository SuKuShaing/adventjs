...
export function isTreesSynchronized(
    ...
    // Si ambos árboles son inexistentes (hemos llegado al final de las ramas), son espejos.
    if (!tree1 && !tree2) return [true, ""];

    // Si uno existe y el otro no, o sus valores son diferentes, NO son espejos.
    if (!tree1 || !tree2 || tree1.value !== tree2.value) {
        return [false, tree1 ? tree1.value : ""];
    }

    // El hijo IZQUIERDO del árbol 1 debe ser espejo del hijo DERECHO del árbol 2
    const [leftOk] = isTreesSynchronized(tree1.left, tree2.right);

    // El hijo DERECHO del árbol 1 debe ser espejo del hijo IZQUIERDO del árbol 2
    const [rightOk] = isTreesSynchronized(tree1.right, tree2.left);

    // Si ambas comparaciones recursivas fueron exitosas, entonces todo el árbol es espejo.
    if (leftOk && rightOk) {

...
function minMovesToStables(reindeer, stables) {
	// ordenar los renos y los establos
    const reindeerSorted = reindeer.sort((a, b) => a - b);
    const stablesSorted = stables.sort((a, b) => a - b);

    // calcular la distancia individuales
    let arrayDeDistancias = [];

    for (let i = 0; i < reindeerSorted.length; i++) {
        arrayDeDistancias.push(Math.abs(reindeerSorted[i] - stablesSorted[i]));
    }

    // sumar las distancias
    return arrayDeDistancias.reduce((acc, curr) => acc + curr);
}
